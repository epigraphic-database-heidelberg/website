...
@bp_data.route('/data/api/geographie/suche', strict_slashes=False)
def api_geographie_search():
    if len(request.args) > 0:
        # create query string
        offset = 0
        limit = 20
        ...
        results = Place.query(query_string, start=offset, number_of_results=limit)
        number_of_hits = results['metadata']['number_of_hits']
        if number_of_hits > 0:
            items_as_list_of_dict = Place.get_items_as_list_of_dicts(results)
        else:
            items_as_list_of_dict = []
        return_dict = {
            "limit" : limit,
